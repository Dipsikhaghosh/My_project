...
<body>
	...
	<div class="breadcrumb-agile py-1">
		<ol class="breadcrumb m-0">
			...
			<li class="breadcrumb-item active" aria-current="page">Find Doctor</li>
		</ol>
	</div>
	<!-- //page details -->
	...
	<div class="gallery py-5" id="gallery">
		<div class="container py-xl-5 py-lg-3">
			<div class="title-section mb-md-5 mb-4">
				<h6 class="w3ls-title-sub">Give to name of the symptom to find the type of doctor</h6>
				<h3 class="w3ls-title text-uppercase text-dark font-weight-bold">SELECT THE SYMPTOM</h3>
				
				<form method="GET" >
					<select name="symptoms" class="form-control" required>
						<option value=''>Select</option>
            				<?php $spec = mysqli_query($con,"SELECT  DISTINCT s_symptoms FROM symptoms");
            				 while($row=mysqli_fetch_array($spec)){?>
                     	<option value="<?php echo $row['s_symptoms']?>"><?php echo $row['s_symptoms']?></option>
                          	<?php } ?>
                    </select>
                    <input type="submit" class="btn submit mt-4" id="submitbtn" name="submitbtn" value="Search">
   				</form>
					<?php 
					if (isset($_GET['submitbtn'])) {
					?>
					<h1>Recomended docs</h1><br>
					<div class="gal-pop-style pt-xl-4">
					<div class="row">
					<?php
						
 						$symptoms=$_GET['symptoms'];
 						 $special = mysqli_query($con,"SELECT s_specialization FROM symptoms WHERE s_symptoms='$symptoms'");
						
						$row=mysqli_fetch_array($special);
 						$specialization=$row['s_specialization'];
 						$getDoc = mysqli_query($con,"SELECT * FROM doctor WHERE d_specialization='$specialization' AND d_status=1 ");
						if(mysqli_num_rows($getDoc) == 0){
							...
							<div class="col-sm-4" style="background-color:lavender;" align="center">
                    			...
                    			<p><a href="findbooking.php?did=<?php echo $row['d_id']?>&name=<?php echo $row['d_name']?>">Book</a></p>
                    		</div>
                    <?php } }  } ?>
